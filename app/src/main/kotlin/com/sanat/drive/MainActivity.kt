...
class MainActivity : Activity() {
    ...
    private fun addNotePage() {
        ...
        val bold = button("B", navy)
        bold.typeface = Typeface.DEFAULT_BOLD

        val underline = button("U", navy)
        underline.paintFlags =
            underline.paintFlags or
        ...
        try {
            when (requestCode) {
                ...
                501 -> {
                    ...
                    if (json != null && store.restoreAll(json)) {
                        renderPage(currentPage)
                        toast("All data restored successfully.")
                    } else {
                        toast("Invalid SKD Data Drive backup.")
                    }
                }
            }
        } catch (e: Exception) {
